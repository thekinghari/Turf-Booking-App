...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const user = new User_1.User({ name, email, password, phone });
        await user.save();
        ...
        const token = jsonwebtoken_1.default.sign({ id: user._id }, process.env.JWT_SECRET || 'fallback_secret', { expiresIn: '7d' });
        res.status(201).json({
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        const resetToken = jsonwebtoken_1.default.sign({ id: user._id }, process.env.JWT_SECRET || 'fallback_secret', { expiresIn: '1h' });
