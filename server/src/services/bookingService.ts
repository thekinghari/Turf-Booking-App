...
export class BookingService {
  ...
  async createBooking(bookingData: any) {
    try {
      // Get turf details
      const turf = await Turf.findById(bookingData.turfId);
      if (!turf) {
  ...
  private async sendBookingConfirmationEmail(to: string, booking: any, turf: any) {
    const subject = 'Booking Confirmation - TurfBook';
  
    // Format time duration
    const duration = Math.floor(
      (new Date(`2023-01-01T${booking.endTime}`).getTime() - 
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <ul style="list-style-type: disc; margin-left: 20px;">
            ${turf.facilities.map((facility: any) => `<li>${facility}</li>`).join('')}
          </ul>
        </div>
  ...
  private async sendBookingConfirmationWhatsApp(to: string, booking: any) {
